  ...
  return (
    <main className="bg-colorGray mt-28">
      <section className="container">
                ...
                {supportLinks.map((item, id) => {
                  return (
                    <Link key={id} href={item.link} className={`${
                      item.link == pathname &&
                      "text-primary transition-all ease-in-out"
                    } transition-all ease-in-out hover:text-primary `}>{item.name}</Link>
                  )
                })}
              </div>
            </article>
        ...
        <div className="container py-2 flex flex-col md:flex-row xl:flex-row justify-between text-colorDark text-center border-t border-colorDark">
